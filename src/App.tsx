...
==================================================`;
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `b737_flight_summary_${aircraftType.toLowerCase()}.txt`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-300 ${isDarkMode ? 'bg-zinc-950 text-zinc-100' : 'bg-zinc-50 text-zinc-900'}`}>
      {/* Header */}
      <header className={`border-b sticky top-0 z-20 shadow-sm transition-colors duration-300 ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-0 sm:h-16 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className={`text-[10px] sm:text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>Royal air Maroc</p>
            </div>
          </div>
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <button 
              ...
              className={`flex items-center justify-center p-2 rounded-lg transition-colors ${isDarkMode ? 'bg-zinc-800 text-amber-400 hover:bg-zinc-700' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200'}`}
            ...
            <button 
              ...
              className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 sm:px-4 py-2 rounded-lg transition-colors text-xs sm:text-sm font-medium ${isDarkMode ? 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200'}`}
            ...
            <button 
              ...
              className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 sm:px-4 py-2 rounded-lg transition-colors text-xs sm:text-sm font-semibold ${isDarkMode ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-zinc-900 text-white hover:bg-zinc-800'}`}
              title="Export clear text weight and balance summary report"
            >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl flex items-center justify-between overflow-hidden relative">
            ...
            <div className="hidden md:block relative z-10 text-right">
              ...
              <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Bottleneck Detected</div>
            </div>
            {/* Background Accent */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 space-y-6">
            <section className={`rounded-2xl shadow-sm border overflow-hidden transition-colors duration-300 ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
              <div className={`px-6 py-4 border-b flex items-center gap-2 ${isDarkMode ? 'border-zinc-800 bg-zinc-800/50' : 'border-zinc-100 bg-zinc-50/50'}`}>
                <Calculator className="w-5 h-5 text-red-600" />
                <h2 className={`font-semibold ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}>Operational Parameters</h2>
              </div>
            ...
            <section className={`rounded-2xl shadow-sm border overflow-hidden transition-colors duration-300 ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
              <div className={`px-6 py-4 border-b flex items-center gap-2 ${isDarkMode ? 'border-zinc-800 bg-zinc-800/50' : 'border-zinc-100 bg-zinc-50/50'}`}>
                ...
                <h2 className={`font-semibold ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}>Fuel Planning</h2>
              </div>
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <InputGroup 
              ...
              <div className={`px-6 py-3 border-t ${isDarkMode ? 'bg-red-950/20 border-zinc-800' : 'bg-red-50/50 border-zinc-100'}`}>
            ...
            <section className={`rounded-2xl shadow-sm border overflow-hidden transition-colors duration-300 ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
              <div className={`px-6 py-4 border-b flex items-center gap-2 ${isDarkMode ? 'border-zinc-800 bg-zinc-800/50' : 'border-zinc-100 bg-zinc-50/50'}`}>
                ...
                <h2 className={`font-semibold ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}>Crew Adjustments</h2>
              </div>
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Aircraft Type Toggle */}
                  <div className="space-y-2">
                    <label className={`text-sm font-medium ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>Aircraft Type</label>
                    <div className={`flex p-1 rounded-lg ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
                      <button 
                        onClick={() => setAircraftType('NG')}
                        className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${aircraftType === 'NG' ? 'bg-red-600 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700'}`}
                      >NG</button>
                      <button 
                        onClick={() => setAircraftType('MAX')}
                        className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${aircraftType === 'MAX' ? 'bg-red-600 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700'}`}
                      >MAX</button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className={`text-sm font-medium ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>Cabin Crew Position</label>
                    <div className={`flex p-1 rounded-lg ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-100'}`}>
                      <button 
                        onClick={() => setCabinPosition('FWD')}
                        className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${cabinPosition === 'FWD' ? 'bg-red-600 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700'}`}
                      >FWD</button>
                      <button 
                        onClick={() => setCabinPosition('AFT')}
                        className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${cabinPosition === 'AFT' ? 'bg-red-600 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700'}`}
                      >AFT</button>
                    </div>
                  </div>
                </div>
              ...
              <div className={`px-6 py-3 border-t ${isDarkMode ? 'bg-zinc-800/50 border-zinc-800' : 'bg-zinc-50 border-zinc-100'}`}>
                <div className={`text-[10px] grid grid-cols-2 gap-x-4 ${isDarkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
                  <div>
                    <span className={`font-bold uppercase ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>Pilot Variations ({aircraftType}):</span>
                  ...
                  <div>
                    <span className={`font-bold uppercase ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>Cabin Variations ({aircraftType}):</span>
                    <ul className="list-disc list-inside ml-1">
            ...
            <section className={`rounded-2xl shadow-sm border overflow-hidden transition-colors duration-300 ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
              <div className={`px-6 py-4 border-b flex items-center gap-2 ${isDarkMode ? 'border-zinc-800 bg-zinc-800/50' : 'border-zinc-100 bg-zinc-50/50'}`}>
                ...
                <h2 className={`font-semibold ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}>Structural Limits</h2>
              </div>
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="sticky top-24">
              ...
              {/* Chart Section */}
              <section className={`rounded-2xl shadow-sm border overflow-hidden mb-6 transition-colors duration-300 ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                <div className={`px-6 py-4 border-b flex items-center justify-between ${isDarkMode ? 'border-zinc-800 bg-zinc-800/50' : 'border-zinc-100 bg-zinc-50/50'}`}>
                  <div className="flex items-center gap-2">
                    <TrendingUp className="w-5 h-5 text-red-600" />
                    <h2 className={`font-semibold ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}>Weight & Balance Envelope</h2>
                  </div>
                </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart layout="vertical" margin={{ top: 10, right: 30, left: 40, bottom: 20 }}>
                      ...
                      <XAxis 
                        ...
                        dataKey="index" 
                        domain={[20, 100]} 
                        label={{ value: 'CG Index', position: 'insideBottom', offset: -10, fontSize: 10, fill: isDarkMode ? '#a1a1aa' : '#71717a' }}
                      ...
                      <YAxis 
                        ...
                        dataKey="weight" 
                        domain={[35000, 85000]} 
                        label={{ value: 'Weight (kg)', angle: -90, position: 'insideLeft', offset: -30, fontSize: 10, fill: isDarkMode ? '#a1a1aa' : '#71717a' }}
                        tick={{ fontSize: 10, fill: isDarkMode ? '#a1a1aa' : '#71717a' }}
                      />
                      <Tooltip 
                        content={({ active, payload }) => {
                          if (active && payload && payload.length) {
                            ...
                            const getPointBg = (name: string) => {
                              ...
                              switch (name) {
                                case 'ZFW': return 'bg-zinc-900 border-blue-900/50';
                                case 'TOW': return 'bg-zinc-900 border-red-900/50';
                                case 'LW': return 'bg-zinc-900 border-emerald-900/50';
                            ...
                            return (
                              <div className={`p-3 rounded-xl shadow-2xl text-xs border transition-all duration-300 ${pointBgClass} ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}>
                      ...
                      <ReferenceLine y={mzfw} stroke="#a1a1aa" strokeDasharray="3 3" label={{ value: 'MZFW', position: 'right', fontSize: 8, fill: '#a1a1aa' }} />
                      <ReferenceLine y={mlw} stroke="#a1a1aa" strokeDasharray="3 3" label={{ value: 'MLW', position: 'right', fontSize: 8, fill: '#a1a1aa' }} />
                ...
                <div className={`px-6 py-3 border-t flex flex-wrap gap-4 text-[10px] transition-colors duration-300 ${isDarkMode ? 'bg-zinc-800/50 border-zinc-800 text-zinc-400' : 'bg-zinc-50 border-zinc-100 text-zinc-500'}`}>
              ...
              <section className={`rounded-2xl shadow-xl overflow-hidden transition-colors duration-300 ${isDarkMode ? 'bg-zinc-900 text-white border border-zinc-800' : 'bg-zinc-900 text-white'}`}>
                <div className={`px-6 py-4 flex items-center justify-between ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-800'}`}>
                  <h2 className="font-semibold flex items-center gap-2">
                    <Info className="w-4 h-4" />
                    Calculation Summary
                  </h2>
                  <span className="text-xs font-mono text-zinc-400">v1.1.0</span>
                </div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="pt-4 border-t border-zinc-800 space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-zinc-400 text-sm">Flight Max Fuel (Limited)</span>
                    ...
                    <div className="p-3 bg-zinc-800/50 rounded-xl border border-zinc-700 space-y-2">
                      <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">Constraint Breakdown</p>
                      
                      <div className="grid grid-cols-1 gap-2">
                        <div className="flex justify-between items-center text-[10px]">
                          <span className="text-zinc-500">Structural Tank Capacity</span>
                          <span className="text-zinc-300 font-mono">{results.maxFuelWeight.toLocaleString()} kg</span>
                        </div>
                        
                        <div className="flex justify-between items-center text-[10px]">
                          <span className="text-zinc-500">MTOW Constraint (MTOW - ZFW)</span>
                          <div className="text-right">
                            <span className={`font-mono ${results.flightMaxFuelByMTOW <= results.maxFuelWeight ? 'text-red-400' : 'text-zinc-300'}`}>
                              {results.flightMaxFuelByMTOW.toLocaleString()} kg
                            </span>
                            <p className="text-[8px] text-zinc-600 leading-none mt-0.5">({mtow.toLocaleString()} - {results.zfw.toLocaleString()})</p>
                          </div>
                        </div>

                        <div className="flex justify-between items-center text-[10px]">
                          <span className="text-zinc-500">MLW Constraint (MLW + Trip - ZFW)</span>
                          <div className="text-right">
                            <span className={`font-mono ${results.flightMaxFuelByMLW <= results.maxFuelWeight ? 'text-red-400' : 'text-zinc-300'}`}>
                              {results.flightMaxFuelByMLW.toLocaleString()} kg
                            </span>
                            <p className="text-[8px] text-zinc-600 leading-none mt-0.5">({mlw.toLocaleString()} + {tripFuel.toLocaleString()} - {results.zfw.toLocaleString()})</p>
                          </div>
                        </div>
                      </div>

                      <p className="text-[9px] text-zinc-500 leading-relaxed mt-2 pt-2 border-t border-zinc-700/50">
                        The current payload of <span className="text-zinc-300">{payload.toLocaleString()} kg</span> results in a ZFW of <span className="text-zinc-300">{results.zfw.toLocaleString()} kg</span>. 
      ...
      <footer className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 border-t transition-colors duration-300 ${isDarkMode ? 'border-zinc-800' : 'border-zinc-100'}`}>
        <div className="flex flex-col items-center gap-6">
          <div className="flex items-center gap-4">
            <div className={`h-px w-12 ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-200'}`} />
            <Plane className="w-4 h-4 text-red-600" />
            <div className={`h-px w-12 ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-200'}`} />
          </div>
          
          <div className="text-center space-y-1">
            <p className={`text-[10px] font-bold uppercase tracking-[0.3em] ${isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>
              Developed by
            </p>
            <h2 className={`text-lg font-bold tracking-tight ${isDarkMode ? 'text-zinc-100' : 'text-zinc-900'}`}>
              Aymane ZBAKH
            </h2>
          </div>

          <div className="flex items-center gap-3 text-zinc-400">
            <span className={`text-[10px] font-mono tracking-wider px-2 py-1 rounded border ${isDarkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-500' : 'bg-zinc-50 border-zinc-100 text-zinc-400'}`}>B737-800 OPS</span>
            <div className={`w-1 h-1 rounded-full ${isDarkMode ? 'bg-zinc-700' : 'bg-zinc-300'}`} />
            <span className={`text-[10px] font-mono tracking-wider px-2 py-1 rounded border ${isDarkMode ? 'bg-zinc-900 border-zinc-800 text-zinc-500' : 'bg-zinc-50 border-zinc-100 text-zinc-400'}`}>v1.0.2</span>
          </div>
        </div>
      </footer>
      ...
      <AnimatePresence>
        {showExportToast && (
          <motion.div
            ...
            className={`fixed bottom-6 right-6 z-50 flex items-center gap-2.5 px-4 py-3 rounded-xl shadow-2xl border text-xs sm:text-sm font-medium ${
              isDarkMode ? 'bg-zinc-900 text-white border-zinc-800' : 'bg-white text-zinc-900 border-zinc-200'
            }`}
          >
            <CheckCircle2 className="w-4 h-4 text-emerald-500 flex-shrink-0" />
            <span>Summary exported & copied to clipboard!</span>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        <label className={`text-sm font-medium flex items-center gap-2 ${error ? 'text-rose-600' : isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>
          {icon && <span className={error ? 'text-rose-400' : isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}>{icon}</span>}
          {label}
        </label>
        <span className={`text-[10px] font-bold uppercase ${isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>{unit}</span>
      </div>
      <div className="relative">
        <input 
          ...
          className={`w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 outline-none transition-all font-mono ${
            isDarkMode ? 'bg-zinc-800 border-zinc-700 text-zinc-100' : 'bg-zinc-50 border-zinc-200 text-zinc-900'
          } ${
            error 
      ...
      {error && errorMsg ? (
        ...
        <p className={`text-[10px] ${isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>{description}</p>
      ) : null}
    </div>
  );
}
...
function ResultCard({ label, value, unit, highlight }: { label: string; value: number | string; unit: string; highlight?: boolean }) {
  return (
    <div className={`p-4 rounded-xl border transition-colors duration-300 ${highlight ? 'bg-red-600 border-red-500' : 'bg-zinc-800 border-zinc-700'}`}>
      <p className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${highlight ? 'text-red-100' : 'text-zinc-400'}`}>{label}</p>
      <div className="flex items-baseline gap-1">
        <span className="text-xl font-mono font-bold text-white">{typeof value === 'number' ? value.toLocaleString() : value}</span>
        <span className={`text-[10px] font-medium ${highlight ? 'text-red-200' : 'text-zinc-500'}`}>{unit}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-end">
        <div>
          <p className={`text-xs mb-0.5 ${isDarkMode ? 'text-zinc-400' : 'text-zinc-400'}`}>{label}</p>
          <div className="flex items-center gap-2">
            <span className={`text-sm font-mono font-bold ${ok ? 'text-white' : 'text-rose-400'}`}>
              {value.toLocaleString()} kg
            </span>
            <span className="text-[10px] text-zinc-500">/ {limit.toLocaleString()} kg</span>
          </div>
          <p className="text-[10px] text-zinc-500 mt-0.5">CG Index: <span className="text-red-400">{index.toFixed(2)}</span></p>
        </div>
      ...
      <div className={`h-1.5 w-full rounded-full overflow-hidden ${isDarkMode ? 'bg-zinc-800' : 'bg-zinc-800'}`}>
        <motion.div 
          initial={{ width: 0 }}
